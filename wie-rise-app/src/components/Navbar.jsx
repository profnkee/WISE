    ...
    return (
      <li className="group relative">
        <button className="flex items-center gap-1 rounded-md px-3 py-2 text-sm font-semibold text-slate-100/90 transition hover:bg-white/10 hover:text-white">
          {item.label}
          <svg className="h-3.5 w-3.5 opacity-70 transition group-hover:rotate-180" viewBox="0 0 20 20" fill="currentColor">
      ...
      <NavLink
        ...
        className={({ isActive }) =>
          `block rounded-md px-3 py-2 text-sm font-semibold transition ${
            isActive ? 'bg-white/15 text-white' : 'text-slate-100/90 hover:bg-white/10 hover:text-white'
          }`
        }
      >
        {item.label}
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <NavLink to="/" className="flex items-center gap-2 shrink-0">
          <img src="/assets/nitlogosc.png" alt="NIT Jamshedpur" className="h-10 w-auto" />
